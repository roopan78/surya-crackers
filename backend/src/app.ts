import express from 'express';
import cors from 'cors';
import helmet from 'helmet';
import compression from 'compression';
import cookieParser from 'cookie-parser';
import morgan from 'morgan';
import { env } from './config/env';
import routes from './routes';
import { serveImageSitemap, serveMerchantFeed, serveSitemap } from './controllers/sitemap.controller';
import { errorHandler, notFoundHandler } from './middleware/error.middleware';

export function createApp() {
  const app = express();

  app.disable('x-powered-by');
  app.use(helmet());
  app.use(
    cors({
      origin: env.corsOrigins,
      credentials: true,
    }),
  );
  app.use(compression());
  app.use(morgan(env.isProduction ? 'combined' : 'dev'));
  // Meta signs webhook payloads over the exact bytes it sent, so the raw buffer
  // has to be kept before JSON.parse discards it. Only retained for webhook
  // paths — no reason to hold a copy of every 5mb admin upload in memory.
  app.use(
    express.json({
      limit: '5mb',
      verify: (req, _res, buffer) => {
        if (req.url?.startsWith('/api/webhooks/')) {
          (req as express.Request).rawBody = Buffer.from(buffer);
        }
      },
    }),
  );
  app.use(cookieParser());

  app.get('/health', (_req, res) => {
    res.json({ success: true, status: 'ok', timestamp: new Date().toISOString() });
  });

  // Not under /api — the storefront domain rewrites these straight here.
  app.get('/sitemap.xml', serveSitemap);
  app.get('/image-sitemap.xml', serveImageSitemap);
  app.get('/feed.xml', serveMerchantFeed);

  app.use('/api', routes);

  app.use(notFoundHandler);
  app.use(errorHandler);

  return app;
}
